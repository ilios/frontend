...
const cohortsHandler = http.get('/api/cohorts', ({ request }) => {
  const url = new URL(request.url);
  const params = url.searchParams;
  const schoolsFilter = params.get('filters[schools]');

  let cohorts = db.cohort.getAll();
...
const coursesHandler = http.get('/api/courses', ({ request }) => {
  const url = new URL(request.url);
  const params = url.searchParams;
...
const sessionsHandler = http.get('/api/sessions', ({ request }) => {
  const url = new URL(request.url);
  const params = url.searchParams;
  const schoolsFilter = params.get('filters[schools]');

  let sessions = db.session.getAll();
...
const pendingUserUpdatesHandler = http.get('/api/pendinguserupdates', ({ request }) => {
  const url = new URL(request.url);
  const params = url.searchParams;
  const schoolsFilter = params.get('filters[schools]');

  let updates = db.pendingUserUpdate.getAll();
...
const programYearCreateHandler = http.post('/api/programyears', async ({ request }) => {
  ...
  const programYear = db.programYear.create(attrs);
...
const userEventsHandler = http.get('/api/userevents/:userid', ({ request, params }) => {
  const url = new URL(request.url);
  const from = Number(url.searchParams.get('from'));
  const to = Number(url.searchParams.get('to'));
  const userid = Number(params.userid);

  const userEvents = db.userevent.getAll().filter((event) => {
...
const schoolEventsHandler = http.get('/api/schoolevents/:schoolid', ({ request, params }) => {
  const url = new URL(request.url);
  const from = Number(url.searchParams.get('from'));
  const to = Number(url.searchParams.get('to'));
  const schoolId = Number(params.schoolid);

  const schoolEvents = db.schoolevent.getAll().filter((event) => {
    const st = DateTime.fromJSDate(event.startDate).toUnixInteger();
    const et = DateTime.fromJSDate(event.endDate).toUnixInteger();
...
// File upload mock
const uploadHandler = http.post('/upload', () => {
  let hash = '';
