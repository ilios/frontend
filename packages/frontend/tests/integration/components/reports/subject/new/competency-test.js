...
/>`);
    ...
    for (let i = 1; i < 5; i++) {
      assert.strictEqual(component.options[i].text, `competency ${i}`);
...
/>`);
    ...
    assert.ok(component.options[0].isSelected);
    assert.strictEqual(component.options[1].text, 'competency 3');
    assert.notOk(component.options[1].isSelected);
    assert.strictEqual(component.options[2].text, 'competency 4');
  ...
  test('changing school resets default value', async function (assert) {
    ...
    await render(
      hbs`<Reports::Subject::New::Competency @currentId={{null}} @changeId={{noop}} @school={{this.school}} />`,
    );
