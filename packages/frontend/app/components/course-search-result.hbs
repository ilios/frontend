  ...
  <ul>
    ...
    {{#each this.sessions as |session|}}
      <li class="session-row">
        <LinkTo @route="session" @models={{array @course.id session.id}} class="session-title-link">
          {{session.title}}
        </LinkTo>
    ...
    {{#if (and (not this.showMore) (gt @course.sessions.length 3))}}
      <button class="show-more link-button" type="button" {{on "click" (set this "showMore" true)}}>
