...
export default Ember.Controller.extend({
  ...
  actions: {
    ...
    goForward(){
      let newDate = moment(this.get('selectedDate')).add(1, this.get('selectedView')).format('DDDDYYYY');
      this.set('date', newDate);
    },
    goBack(){
      let newDate = moment(this.get('selectedDate')).subtract(1, this.get('selectedView')).format('DDDDYYYY');
      this.set('date', newDate);
    },
    gotoToday(){
