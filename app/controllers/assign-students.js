...
export default Controller.extend({
  ...
  unassignedStudents: computed('schoolId', 'selectedSchool', async function () {
    const filters = {
      cohorts: null,
      enabled: true,
  ...
  filteredUnassignedStudents: computed('query', 'unassignedStudents', async function () {
    const query = this.query;
    const students = await this.unassignedStudents;
    const sortedStudents = students.sortBy('fullName');
    return isPresent(query) ? this.filterStudents(sortedStudents, query) : sortedStudents;
  }),
