...
test('objective list', function() {
  ...
  andThen(function() {
    ...
    var secondRow = find('.container table:first tbody tr:eq(1)');
...
test('create new', function() {
  ...
  andThen(function() {
    var row = find('.container table:first tbody tr:eq(2)');
    ...
    fillIn('.container table:first tbody tr:eq(2) td:eq(0) textarea', 'aaa Test');
    click('button:contains("Done")', row).then(function(){
      ...
      // equal(find('.container table:first tbody tr:eq(0) td:eq(0)').text().trim(), 'aaa Test');
      // equal(find('.container table:first tbody tr:eq(1) td:eq(0)').text().trim(), 'First Objective');
    });
  });
});
...
test('edit', function() {
  ...
  andThen(function() {
    ...
    fillIn('.container table:first tbody tr:eq(0) td:eq(0) textarea', 'xxxEdited Objective');
    ...
    click('td:eq(3) button', row).then(function(){
      equal(find('.container table:first tbody tr:eq(1) td:eq(0)').text().trim(), 'xxxEdited Objective');
      equal(find('.container table:first tbody tr:eq(1) td:eq(1)').text().trim(), 'First Child Competency');
    });
  });
});
