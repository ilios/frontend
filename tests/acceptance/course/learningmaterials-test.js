...
import { settled, find, click, fillIn, findAll, currentPath, triggerEvent, visit } from '@ember/test-helpers';
...
module('Acceptance: Course - Learning Materials', function(hooks) {
  ...
  test('list learning materials', async function (assert) {
    ...
    assert.equal(findAll(courseLearningMaterials).length, 4);
  ...
  test('view url file learning material details', async function(assert) {
    ...
    assert.equal(find('.downloadurl a', container).getAttribute('href'), 'http://example.com/file');
    assert.equal(findAll('.citation', container).length, 0);
    assert.equal(findAll('.link', container).length, 0);
  });
  ...
  test('view link learning material details', async function(assert) {
    ...
    assert.equal(findAll('.copyrightpermission', container).length, 0);
    assert.equal(findAll('.copyrightrationale', container).length, 0);
    assert.equal(findAll('.citation', container).length, 0);
    assert.equal(findAll('.copyrightpermission', container).length, 0);
  });
  ...
  test('cancel editing learning material', async function(assert) {
    ...
    assert.equal(getElementText(find(findAll('.detail-learningmaterials .learning-material-table tbody tr:eq(0) td')[5])), getText('status 0'));
    await settled();
  });
