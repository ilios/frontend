...
module('Acceptance | Course - Objective List', function(hooks) {
  ...
  test('list objectives', async function(assert) {
    ...
    const competencies = this.server.createList('competency', 2, { school: this.school });
    ...
    assert.equal(page.objectives.objectiveList.objectives[1].parents.list[0].text, 'program-year objective 1');
    ...
    for (let i=2; i <= 12; i++) {
      assert.equal(page.objectives.objectiveList.objectives[i].description.text, `course objective ${i}`);
