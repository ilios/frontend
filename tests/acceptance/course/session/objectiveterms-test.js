...
module('Acceptance | Session - Objective Vocabulary Terms', function(hooks) {
  ...
  hooks.beforeEach(async function () {
    ...
    this.server.create('session-objective', { session, objective, terms: [ term ] });
    this.school = school;
  });
  ...
  test('manage and save terms', async function(assert) {
    ...
    assert.equal(page.objectives.objectiveList.objectives[0].taxonomyManager.selectedTerms.length, 1);
    assert.equal(
      page.objectives.objectiveList.objectives[0].taxonomyManager.selectedTerms[0].title,
      'Vocabulary 1 (school 0)'
    );
    assert.equal(page.objectives.objectiveList.objectives[0].taxonomyManager.selectedTerms[0].terms.length, 1);
    assert.equal(page.objectives.objectiveList.objectives[0].taxonomyManager.selectedTerms[0].terms[0].name, 'term 0');
    assert.equal(page.objectives.objectiveList.objectives[0].taxonomyManager.vocabulary.options.length, 1);
    assert.equal(page.objectives.objectiveList.objectives[0].taxonomyManager.vocabulary.options[0].value, '1');
    assert.equal(
      page.objectives.objectiveList.objectives[0].taxonomyManager.vocabulary.options[0].text,
      'Vocabulary 1 (school 0)');
    assert.ok(page.objectives.objectiveList.objectives[0].taxonomyManager.vocabulary.options[0].isSelected);

    assert.equal(page.objectives.objectiveList.objectives[0].taxonomyManager.availableTerms.length, 4);
    assert.equal(page.objectives.objectiveList.objectives[0].taxonomyManager.availableTerms[0].name, 'term 0');
    assert.ok(page.objectives.objectiveList.objectives[0].taxonomyManager.availableTerms[0].isSelected);
    assert.notOk(page.objectives.objectiveList.objectives[0].taxonomyManager.availableTerms[1].isSelected);
    assert.notOk(page.objectives.objectiveList.objectives[0].taxonomyManager.availableTerms[2].isSelected);
    assert.notOk(page.objectives.objectiveList.objectives[0].taxonomyManager.availableTerms[3].isSelected);
    ...
    assert.equal(page.objectives.objectiveList.objectives[0].selectedTerms.list[0].terms.length, 1);
    assert.equal(page.objectives.objectiveList.objectives[0].selectedTerms.list[0].terms[0].name, 'term 3');
  });
