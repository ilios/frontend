...
module('Acceptance | Session - Objective List', function(hooks) {
  ...
  test('list objectives', async function(assert) {
    ...
    assert.equal(page.objectives.objectiveList.objectives[1].parents.list[0].text, 'course objective 1');
    ...
    for (let i=2; i <= 12; i++) {
      assert.equal(page.objectives.objectiveList.objectives[i].description.text, `session objective ${i}`);
