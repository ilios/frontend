...
module('Unit | Model | LearnerGroup', function(hooks) {
  ...
  test('it exists', function(assert) {
    let model = run(() => this.owner.lookup('service:store').createRecord('learner-group'));
  ...
  test('list courses', async function(assert) {
    ...
    const model = run(() => store.createRecord('learner-group'));
    await run( async () => {
  ...
  test('check allDescendantUsers on populated group with sub-groups', async function(assert) {
    ...
    let learnerGroup = run(() => store.createRecord('learner-group'));

    let user1 = run(() => store.createRecord('user'));
    let user2 = run(() => store.createRecord('user'));
    let user3 = run(() => store.createRecord('user'));
    let user4 = run(() => store.createRecord('user'));
    let user5 = run(() => store.createRecord('user'));
    let subGroup1 = run(() => store.createRecord('learner-group', {users: [user2]}));
    let subSubGroup1 = run(() => store.createRecord('learner-group', {users: [user3]}));
    let subGroup2 = run(() => store.createRecord('learner-group', {users: [user5], children: [ subSubGroup1 ]}));
  ...
  test('check allinstructors', async function(assert) {
    assert.expect(8);
    const store = this.owner.lookup('service:store');
    const learnerGroup = run(() => store.createRecord('learner-group'));
  ...
  test('check sortTitle on sub group', async function(assert) {
    assert.expect(3);
    const store = this.owner.lookup('service:store');
    const learnerGroup = run(() => store.createRecord('learner-group', { id: 1, title: 'top group' }));
    let groups = await learnerGroup.get('allDescendants');
    assert.equal(groups.length, 0);

    const subGroup1 = run(() => store.createRecord('learner-group', {id: 2, parent: learnerGroup, title: 'subGroup1'}));
    const subGroup2 = run(() => store.createRecord('learner-group', {id: 3, parent: subGroup1, title: 'subGroup2'}));
    const subGroup3 = run(() => store.createRecord('learner-group', {id: 4, parent: subGroup2, title: 'subGroup3'}));
  ...
  test('check addUserToGroupAndAllParents', async function(assert) {
    ...
    const subGroup1 = run(() => store.createRecord('learner-group', {parent: learnerGroup, users: [user1]}));
    const subGroup2 = run(() => store.createRecord('learner-group', {parent: subGroup1}));
    const subGroup3 = run(() => store.createRecord('learner-group', {parent: subGroup2}));
    const subGroup4 = run(() => store.createRecord('learner-group', {parent: subGroup1}));
  ...
  test('has no learners in group without learners and without subgroups', async function(assert) {
    assert.expect(1);
    const learnerGroup = run(() => this.owner.lookup('service:store').createRecord('learner-group'));
    await run( async () => {
  ...
  test('allParentTitles', async function(assert) {
    assert.expect(4);
    const store = this.owner.lookup('service:store');
    const learnerGroup = run(() => store.createRecord('learner-group', {title: 'Foo'}));
    let titles = await learnerGroup.get('allParentTitles');
    assert.equal(titles.length, 0);

    const subGroup = run(() => store.createRecord('learner-group', {title: 'Bar', parent: learnerGroup }));
    const subSubGroup = run(() => store.createRecord('learner-group', {title: 'Baz', parent: subGroup }));
  ...
  test('isTopLevelGroup', async function(assert) {
    assert.expect(2);
    const store = this.owner.lookup('service:store');
    const learnerGroup = run(() => store.createRecord('learner-group', { id: 1 }));
    assert.ok(learnerGroup.get('isTopLevelGroup'));

    const subGroup = run(() => store.createRecord('learner-group', { parent: learnerGroup}));
