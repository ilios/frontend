...
module('Unit | Model | Cohort', function (hooks) {
  ...
  test('get root level groups', async function (assert) {
    assert.expect(3);
    ...
    const topLevelGroups = await model.getRootLevelLearnerGroups();
  ...
  test('get competencies', async function (assert) {
    assert.expect(3);
    const model = this.owner.lookup('service:store').createRecord('cohort');
    const store = this.owner.lookup('service:store');
    const competency1 = store.createRecord('competency');
    ...
    model.set('programYear', programYear);
    const competencies = await model.getCompetencies();
    assert.equal(competencies.length, 2);
    assert.ok(competencies.includes(competency1));
    assert.ok(competencies.includes(competency2));
  });

  test('get program', async function (assert) {
    assert.expect(1);
    const model = this.owner.lookup('service:store').createRecord('cohort');
    const store = this.owner.lookup('service:store');
    const program1 = store.createRecord('program');
    ...
    model.set('programYear', programYear);
    const program = await model.getProgram();
    assert.equal(program, program1);
  });

  test('get school', async function (assert) {
    assert.expect(1);
    const model = this.owner.lookup('service:store').createRecord('cohort');
    const store = this.owner.lookup('service:store');
    const school1 = store.createRecord('school');
    ...
    const programYear = store.createRecord('program-year', { program });
    model.set('programYear', programYear);
    const school = await model.getSchool();
    assert.equal(school, school1);
  });

  test('get classOfYear', async function (assert) {
    assert.expect(1);
    const model = this.owner.lookup('service:store').createRecord('cohort');
    const store = this.owner.lookup('service:store');
    const program = store.createRecord('program', { duration: 4 });
    ...
    model.set('programYear', programYear);
    const classOfYear = await model.getClassOfYear();
    assert.equal(classOfYear, 2016);
  });
});
