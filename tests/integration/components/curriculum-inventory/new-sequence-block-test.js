...
module('Integration | Component | curriculum-inventory/new-sequence-block', function (hooks) {
  ...
  test('order-in-sequence options are visible for ordered parent sequence block', async function (assert) {
    ...
    await render(
      hbs`<CurriculumInventory::NewSequenceBlock @report={{report}} @parent={{parentBlock}} />`
    );
  ...
  test('save with defaults', async function (assert) {
    ...
    await render(
      hbs`<CurriculumInventory::NewSequenceBlock @report={{report}} @save={{action saveBlock}} />`
    );

    await fillIn('.title input', newTitle);
    await fillIn('.description textarea', newDescription);
    await click('.start-date input');
    find('[data-test-start-date-picker]')._flatpickr.setDate(newStartDate.toDate(), true);
  ...
  test('cancel', async function (assert) {
    ...
    await render(
      hbs`<CurriculumInventory::NewSequenceBlock @report={{report}} @cancel={{action cancelAction}} />`
    );
