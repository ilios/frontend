...
module(
  ...
  function (hooks) {
    ...
    test('it renders', async function (assert) {
      ...
      assert.strictEqual(component.table.headings.length, 5);
      assert.strictEqual(component.table.headings[0].text, 'Non-Clerkship Sequence Blocks');
      assert.strictEqual(component.table.headings[1].text, 'Start Level');
      assert.strictEqual(component.table.headings[2].text, 'End Level');
      assert.strictEqual(component.table.headings[3].text, 'Total Weeks');
      assert.strictEqual(component.table.headings[4].text, 'Average Hours of Instruction Per Week');
      assert.strictEqual(component.table.rows.length, 2);
      assert.strictEqual(component.table.rows[0].title, 'foo');
      assert.strictEqual(component.table.rows[0].startLevel, '2');
      assert.strictEqual(component.table.rows[0].endLevel, '3');
      ...
      assert.strictEqual(component.table.rows[1].startLevel, '4');
      assert.strictEqual(component.table.rows[1].endLevel, '5');
