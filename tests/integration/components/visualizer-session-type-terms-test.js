...
module('Integration | Component | visualizer session type terms', function(hooks) {
  ...
  test('it renders', async function(assert) {
    ...
    const vocabularyModel = await this.owner.lookup('service:store').find('session-type', vocabulary.id);
    this.set('vocabulary', vocabularyModel);
    await render(hbs`<VisualizerSessionTypeTerms @sessionType={{this.sessionType}} @vocabulary={{this.vocabulary}} />`);
