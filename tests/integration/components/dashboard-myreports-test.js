...
let mockReports = [
  Ember.Object.create({title: 'all courses', subject: 'courses', user: 1}),
  Ember.Object.create({title: 'courses for session', subject: 'courses', prepositionalObject: 'session', prepositionalObjectTableRowId: 11, user: 1}),
];
...
test('list reports', function(assert) {
  ...
  for(let i = 0; i < 2; i++){
    ...
    assert.equal(tds.eq(0).text().trim(), mockReports[i].title);
  }
