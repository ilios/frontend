...
module(
  'Integration | Component | ilios calendar single event objective list',
  function (hooks) {
    ...
    />`);
      ...
      assert
        .dom('ul.list-in-order')
        .doesNotExist('Objectives list is not visible');
      assert.dom('ul.tree>li').hasText('annoying things buying gas traffic');
      assert
        .dom('ul.tree>li:nth-of-type(1)>ul>li:nth-of-type(1)')
        .hasText('buying gas');
      assert
        .dom('ul.tree>li:nth-of-type(1)>ul>li:nth-of-type(2)')
        .hasText('traffic');
      assert
        .dom('ul.tree>li:nth-of-type(2)')
        .hasText('great things cheese ice cream');
      assert
        .dom('ul.tree>li:nth-of-type(2)>ul>li:nth-of-type(1)')
        .hasText('cheese');
      assert
        .dom('ul.tree>li:nth-of-type(2)>ul>li:nth-of-type(2)')
        .hasText('ice cream');
      assert
        .dom('h2 button')
        .hasClass('active', 'Display-mode button is visible and is "active"');
      await click('h2 button');

      assert
        .dom('ul.tree')
        .doesNotExist('Domains/Objectives tree is not visible');
      assert.dom('ul.list-in-order').exists('Objectives list is visible');
      for (let i = 0, n = objectives.length; i < n; i++) {
        assert
          .dom(`.list-in-order li:nth-of-type(${i + 1})`)
          .hasText(
            `${objectives[i].title} ${objectives[i].domain}`,
            'Objective title is visible'
          );
        assert
    ...
    test('displays `None` when provided no content', async function (assert) {
      ...
      this.set('objectives', []);
      await render(
        hbs`<SingleEventObjectiveList @objectives={{this.objectives}} />`
      );
    ...
    />`);
      ...
      assert
        .dom('h2 button')
        .doesNotExist('Display-mode button is not visible');
