...
module('Integration | Component | school competencies list', function (hooks) {
  ...
  test('it renders', async function (assert) {
    ...
    assert.equal(component.items[0].title.text, 'domain 0');
    assert.equal(component.items[0].pcrs.items.length, 1);
    assert.equal(component.items[0].pcrs.items[0].text, 'None');
    assert.ok(component.items[0].title.isDomain);
    assert.equal(component.items[1].title.text, 'competency 0');
    assert.equal(component.items[1].pcrs.items.length, 2);
    assert.equal(component.items[1].pcrs.items[0].text, '1 aamc pcrs 0');
    assert.equal(component.items[1].pcrs.items[1].text, '2 aamc pcrs 1');
    assert.ok(component.items[1].title.isCompetency);
    assert.equal(component.items[2].title.text, 'competency 1');
    assert.ok(component.items[2].title.isCompetency);
    assert.equal(component.items[2].pcrs.items.length, 1);
    assert.equal(component.items[2].pcrs.items[0].text, '3 aamc pcrs 2');
