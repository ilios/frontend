...
module('Integration | Component | curriculum inventory sequence block details', function(hooks) {
  ...
  test('it renders', async function(assert) {
    ...
    for (let i = 0; i < 10; i++) {
      academicLevels.pushObject(this.server.create('curriculumInventoryAcademicLevel', { name: `Year ${i + 1}` }));
    }
    const program = this.server.create('program', { school });
    const report = this.server.create('curriculumInventoryReport', { academicLevels, year: '2016', program });
    const grandParentBlock = this.server.create('curriculumInventorySequenceBlock', { title: 'Okely Dokely', report });
    ...
    const parentBlockModel = await this.owner.lookup('service:store').find('curriculum-inventory-sequence-block', parentBlock.id);
    const grandParentBlockModel = await this.owner.lookup('service:store').find('curriculum-inventory-sequence-block', grandParentBlock.id);
    ...
    assert.dom('.curriculum-inventory-sequence-block-header .title').hasText(
      blockModel.get('title'), 'Block header is visible.'
    );
    assert.dom('.curriculum-inventory-sequence-block-overview .description .editinplace').hasText(
      blockModel.get('description'), 'Block overview is visible.'
    );
