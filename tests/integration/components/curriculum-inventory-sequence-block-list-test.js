...
module('Integration | Component | curriculum inventory sequence block list', function(hooks) {
  ...
  test('it renders with nested blocks', async function(assert) {
    ...
    return settled().then(() => {
      ...
      assert.dom(findAll('tbody tr:nth-of-type(1) td')[2]).hasText(block1.get('orderInSequence').toString());
      ...
      assert.dom(findAll('tbody tr:nth-of-type(2) td')[1]).hasText(academicLevel2.get('level').toString());
      assert.dom(findAll('tbody tr:nth-of-type(2) td')[2]).hasText(block2.get('orderInSequence').toString());
