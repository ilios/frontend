...
module('Integration | Component | curriculum inventory sequence block list', function(hooks) {
  ...
  test('it renders with top-level sequence blocks', async function(assert) {
    ...
    await render(hbs`{{curriculum-inventory-sequence-block-list report=report remove='removeSequenceBlock'}}`);
    return settled().then(() => {
  ...
  test('it renders with nested blocks', async function(assert) {
    ...
    this.set('removeSequenceBlock', function() {});
    await render(hbs`{{curriculum-inventory-sequence-block-list parent=parent remove='removeSequenceBlock'}}`);
    return settled().then(() => {
  ...
  test('cancel delete', async function(assert){
    ...
    this.set('report', report);
    await render(hbs`{{curriculum-inventory-sequence-block-list report=report}}`);
    await this.$('tbody tr:eq(0) td:eq(6) .remove').click();
    await settled();
  ...
  test('empty nested blocks list', async function(assert) {
    ...
    this.set('parent', parentBlock);
    await render(hbs`{{curriculum-inventory-sequence-block-list parent=parent}}`);
    return settled().then(() => {
      assert.equal(this.$('.title').text().trim(), `Sequence Blocks (0)`,
        'Component title is correct, and show the correct number of blocks.'
      );
