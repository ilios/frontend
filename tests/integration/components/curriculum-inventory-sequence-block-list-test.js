...
module('Integration | Component | curriculum inventory sequence block list', function (hooks) {
  ...
  test('it renders with top-level sequence blocks', async function (assert) {
    ...
    const academicLevel1 = EmberObject.create({ id: 1, level: '2' });
    const academicLevel2 = EmberObject.create({ id: 2, level: '1' });

    const block1 = EmberObject.create({
      id: 1,
    ...
    const block2 = EmberObject.create({
      id: 2,
    ...
    return settled().then(() => {
      ...
      assert
        ...
        .hasText('Sequence #', 'Table column header has correct label.');
      assert
        .dom(findAll('thead th')[3])
        .hasText('Start Date', 'Table column header has correct label.');
      assert
        .dom(findAll('thead th')[4])
        .hasText('End Date', 'Table column header has correct label.');
      assert
        .dom(findAll('thead th')[5])
        .hasText('Course', 'Table column header has correct label.');
      assert
        .dom(findAll('thead th')[6])
        .hasText('Actions', 'Table column header has correct label.');
      ...
      assert
        .dom('tbody tr:nth-of-type(1) td:nth-of-type(7) .edit')
        .exists({ count: 1 }, 'Edit link is visible.');
      assert
        .dom('tbody tr:nth-of-type(1) td:nth-of-type(7) .remove')
        .exists({ count: 1 }, 'Remove link is visible.');
      ...
      assert
        ...
        .hasText(moment(block1.get('startDate')).format('L'));
      assert
        .dom(findAll('tbody tr:nth-of-type(2) td')[4])
        .hasText(moment(block1.get('endDate')).format('L'));
      assert.dom(findAll('tbody tr:nth-of-type(2) td')[5]).hasText(course.get('title'));
      assert
        .dom('tbody tr:nth-of-type(2) td:nth-of-type(7) .edit')
        .exists({ count: 1 }, 'Edit link is visible.');
      assert
        .dom('tbody tr:nth-of-type(2) td:nth-of-type(7) .remove')
        .exists({ count: 1 }, 'Remove link is visible.');
    });
  });
  ...
  test('it renders with nested blocks', async function (assert) {
    ...
    const nestedBlocks = [block1, block2];
    ...
    return settled().then(() => {
      ...
      assert
        ...
        .hasText(block1.get('orderInSequence').toString());
      assert
      ...
      assert
        ...
        .hasText(academicLevel2.get('level').toString());
      assert
        .dom(findAll('tbody tr:nth-of-type(2) td')[2])
        .hasText(block2.get('orderInSequence').toString());
  ...
  test('read-only mode', async function (assert) {
    ...
    return settled().then(() => {
      ...
      assert
        .dom('tbody tr:nth-of-type(1) td:nth-of-type(7) .remove')
        .doesNotExist('Remove link is not visible.');
    });
  });
  ...
  test('delete', async function (assert) {
    ...
    assert.equal(
      find('tbody tr:nth-of-type(2) .confirm-message')
        .textContent.trim()
        .indexOf('Are you sure you want to delete'),
      0,
      'Confirmation message is visible.'
    );
    assert
      .dom('tbody tr:nth-of-type(2) .confirm-buttons .remove')
      .exists({ count: 1 }, 'Delete button is visible.');
    assert
      .dom('tbody tr:nth-of-type(2) .confirm-buttons .done')
      .exists({ count: 1 }, 'Cancel button is visible.');
  ...
  test('cancel delete', async function (assert) {
    ...
    assert
      .dom('tbody .confirm-message')
      .doesNotExist('Confirmation dialog is not visible after cancelling.');
  });
  ...
  test('empty top level blocks list', async function (assert) {
    ...
    this.set('report', report);
    await render(
      hbs`<CurriculumInventorySequenceBlockList @report={{report}} @canUpdate={{true}} />`
    );
    return settled().then(() => {
      assert
        .dom('.title')
      ...
      assert
        .dom('.default-message')
        .hasText('There are no sequence blocks in this report.', 'No blocks message is visible.');
    });
  });
