...
module('Integration | Component | new session', function(hooks) {
  ...
  hooks.beforeEach(async function() {
    ...
    this.sessionType2 = await this.owner.lookup('service:store').find('session-type', sessionType2.id);
  });

  test('it renders', async function(assert) {
    this.set('sessionTypes', [ this.sessionType, this.sessionType2 ]);
    await render(hbs`<NewSession @save={{noop}} @cancel={{noop}} @sessionTypes={{this.sessionTypes}} />`);
  ...
  test('cancel', async function(assert) {
    ...
    await render(hbs`<NewSession @save={{noop}} @cancel={{this.cancel}} @sessionTypes={{array}} />`);
  ...
  test('save', async function(assert) {
    ...
    await render(hbs`<NewSession @save={{this.save}} @cancel={{noop}} @sessionTypes={{this.sessionTypes}} />`);
