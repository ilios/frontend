  ...
  test('change academic year', async function(assert) {
    ...
    return settled().then(async () => {
      ...
      return settled().then(async () => {
        ...
        assert.dom('.academic-year option:checked').hasValue(report.get('year').toString(), "The report's year should be selected.");
        ...
        return settled().then(() => {
          ...
          assert.equal(report.get('year'), newVal.toString(), 'Report year got updated with new value.');
        });
      });
    });
  });
