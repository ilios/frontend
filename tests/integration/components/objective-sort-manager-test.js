...
module(
  'Integration | Component | objective sort manager',
  async function (hooks) {
    ...
    test('it renders for session', async function (assert) {
      ...
      await render(
        hbs`<ObjectiveSortManager @subject={{this.subject}} @close={{noop}} />`
      );
      assert.dom('.draggable-object').exists({ count: 2 });
      assert.dom('.draggable-object').hasText('session objective 1');
      assert
        .dom(findAll('.draggable-object')[1])
        .hasText('session objective 0');
    ...
    test('it renders for program-year', async function (assert) {
      ...
      assert
        .dom(findAll('.draggable-object')[1])
        .hasText('program-year objective 0');
      assert.dom('.actions .bigadd').exists({ count: 1 });
      assert.dom('.actions .bigcancel').exists({ count: 1 });
    });
    ...
    test('cancel', async function (assert) {
      ...
      await render(
        hbs`<ObjectiveSortManager @subject={{this.subject}} @close={{this.cancel}} />`
      );
