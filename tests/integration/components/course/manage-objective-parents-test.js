...
module(
  'Integration | Component | course/manage-objective-parents',
  function (hooks) {
    ...
    />`);
      ...
      assert.equal(
        component.competencies[0].objectives[0].title,
    ...
    test('inactive parents are hidden unless they are selected', async function (assert) {
      const activeProgramYearObjective = this.server.create(
        'programYearObjective'
      );
      const inactiveProgramYearObjective = this.server.create(
        'programYearObjective',
        { active: false }
      );
      const inactiveSelectedProgramYearObjective = this.server.create(
        'programYearObjective',
        { active: false }
      );
    ...
    />`);
      ...
      assert.ok(component.competencies[0].objectives[0].notSelected);
      assert.equal(
        component.competencies[0].objectives[1].title,
