...
module(
  'Integration | Component | learning materials sort manager',
  function (hooks) {
    ...
    test('it renders', async function (assert) {
      ...
      return settled().then(() => {
        ...
        assert
          .dom('.draggable-object:nth-of-type(1) [data-test-title]')
          .hasText(lm2.get('title'));
        assert
          .dom('.draggable-object:nth-of-type(1) .lm-type-icon .fa-paragraph')
          .exists({ count: 1 }, 'Shows LM type icon.');

        assert.equal(
          find('.draggable-object:nth-of-type(1) .details').textContent.replace(
            /[\s\n\t]+/g,
            ''
          ),
        ...
        assert
          .dom('.draggable-object:nth-of-type(2) [data-test-title]')
          .hasText(lm1.get('title'));
        assert
          .dom('.draggable-object:nth-of-type(2) .lm-type-icon .fa-file-pdf')
          .exists({ count: 1 }, 'Shows LM type icon.');

        assert.equal(
          find('.draggable-object:nth-of-type(2) .details').textContent.replace(
            /[\s\n\t]+/g,
            ''
          ),
