...
module('Integration | Service | school config', function (hooks) {
  ...
  test('missing or wrong school ids return undefined or default value', async function (assert) {
    const school = await this.server.create('school');
  ...
  test('save() sends PATCH only for dirty existing configurations', async function (assert) {
    ...
    this.server.patch(
      '/api/schoolconfigs/:id',
      async ({ params, request }) => {
        const { data } = await request.json();
        assert.step(`${data.attributes.name} PATCH called`);

        const config = this.server.db.schoolConfig.all().find(({ id }) => id === Number(params.id));
        config.value = data.attributes.value;
        return formatJsonApi(config, 'schoolConfig');
      },
      false,
    );
