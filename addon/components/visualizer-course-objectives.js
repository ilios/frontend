...
export default class VisualizerCourseObjectives extends Component {
  ...
  *load(element, [course]) {
    ...
    const sessionCourseObjectiveMap = yield map(
      sessions.toArray(),
      async (session) => {
        ...
        const flatObjectives = courseSessionObjectives.reduce(
          (flattened, obj) => {
    ...
    const mappedObjectives = courseObjectives
      .toArray()
      .map((courseObjective) => {
    ...
    this.objectiveWithMinutes = condensedObjectiveData.filter(
      (obj) => obj.data !== 0
    );
